...
<script lang="ts">
	...
	$: {
		open = add_new_question_popup_open;
	}
	...
	<aside class={` w-full h-full bg-gray-200 border-r-2 shadow-lg z-20 lg:hidden fixed top-0 left-0 `} class:open >
		<div class="border-r-2 pt-6 px-6 overflow-scroll h-full">
			...
			>
				...
				>
					<p
						type="text"
						class="whitespace-nowrap truncate text-center w-full bg-transparent text-black rounded dark:text-white"
			...
			<div
				class="bg-white shadow rounded-lg min-h-40 p-2 hover:cursor-pointer drop-shadow-2xl border border-gray-500 dark:bg-gray-600 grid grid-cols-2"
			>
				<button
					type="button"
	...
	<div class="h-screen relative lg:block hidden">
