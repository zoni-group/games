...
<div class="flex justify-center">
	<div class="grid grid-cols-4 w-11/12 gap-4">
		<div class="border-[#004A93] border-2 rounded">
			<h2 class="text-center">{$t('quiztivity.memory.editor.add_card')}</h2>
			<div class="grid grid-cols-2 py-2">
				<div class="px-2 flex flex-col gap-2">
					<textarea
						type="text"
						class="h-auto resize-none bg-transparent outline-none rounded outline-[#004A93] outline"
						rows="3"
		...
		{#each data.cards as card_pair, i (card_pair[0].id)}
			<div
				class="border-[#004A93] border-2 rounded group flex flex-col"
