	...
	<div class="flex justify-center w-full">
		<div class="w-11/12 m-auto">
			{#if data.results.length === 0}
				<p class="text-center text-white text-3xl mt-8">{$t('results_page.no_results_so_far')}</p>
			{:else}
				<table class="w-full">
					<tr class="border-b-2 dark:border-gray-500 text-left border-gray-300">
						<th class="border-r text-white dark:border-gray-500 p-1 mx-auto border-gray-300"
						...
						<th class="mx-auto text-white p-1">{$t('words.note')}</th>
					</tr>
					{#each data.results as result}
						<tr class="text-left">
							<td class="border-r text-white dark:border-gray-500 p-1 border-gray-300"
