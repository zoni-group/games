...
{:then _}
	<div class="grid grid-rows-2 h-screen">
		<div class="grid grid-cols-2 h-full border-b-2 border-[#00529B] dark:border-white">
			<div class="h-full w-full border-r-2 border-[#00529B] dark:border-white">
				<h2 class="text-center text-[#0056BD] dark:text-white text-2xl">{$t('security_settings.backup_code')}</h2>
			...
			<div class="h-full w-full">
				<h2 class="text-center text-2xl text-[#0056BD] dark:text-white">{$t('security_settings.activate_2fa')}</h2>
				<div
					...
					<div class="m-auto">
						{#if user_data.require_password}
							<div class="flex items-center space-x-2">
								...
								<span class="text-sm font-medium text-[#0056BD] dark:text-white"
		...
		<div class="grid grid-cols-2 h-full">
			<div class="h-full w-full flex flex-col border-r-2 border-[#00529B] dark:border-white">
				<h2 class="text-center text-2xl text-[#0056BD] dark:text-white">{$t('security_settings.webauthn')}</h2>
				<div class="flex justify-center ">
					{#if security_keys.length > 0}
						<p class="text-[#0056BD] dark:text-white">{$t('security_settings.webauthn_available')}</p>
					{:else}
						<p class="text-[#0056BD] dark:text-white">{$t('security_settings.webauthn_unavailable')}</p>
					{/if}
				</div>
				<div class="flex justify-center">
					<div class="m-auto">
						...
						{#each security_keys as key, i}
							<li>
								<button
									...
									class="hover:line-through transition text-[#0056BD] dark:text-white">{i + 1}</button
								>
							</li>
						{/each}
					</ul>
			...
			<div class="h-full w-full flex flex-col">
				<h2 class="text-center text-2xl text-[#0056BD] dark:text-white">{$t('security_settings.totp')}</h2>
				<div class="flex justify-center">
					{#if totp_activated}
						<p class="text-[#0056BD] dark:text-white">{$t('security_settings.totp_available')}</p>
					{:else}
						<p class="text-[#0056BD] dark:text-white">{$t('security_settings.totp_unavailable')}</p>
					{/if}
				</div>
